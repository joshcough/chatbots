...
withStream :: (StreamsDb m, MonadError ChatBotError m) => ChannelName -> (Stream -> m a) -> m a
...
instance (HasConfig c, MonadIO m) => CommandsDb (AppT' ChatBotError m c) where
  ...
  getCommand c commandName = do
    ...
    withStream c $ \s -> fmap (fmap (dbCommandToCommand s)) . runDb $ selectFirst
      [DbCommandChannel P.==. toSqlKey (_streamId s), DbCommandName P.==. commandName]
      []
  deleteCommand c commandName = do
    let channel = getChannelName c
    $(logDebug) "deleteCommand" ["channel" .= channel, "name" .= commandName]
    withStream c $ \s -> runDb $ delete $ from $ \command -> where_
      ((command ^. DbCommandChannel)
      ==. val (toSqlKey $ _streamId s)
      &&. (command ^. DbCommandName)
      ==. val commandName
      )
  getCommands c = do
    let channel = getChannelName c
    $(logDebug) "getCommands" ["channel" .= channel]
    withStream c $ \s -> fmap (fmap (dbCommandToCommand s)) . runDb $ select $ from $ \command -> do
  ...
  deleteQuote c qid = do
    ...
    withStream c $ \s ->
        -- delete from quote where channel = channel and qid = qid
                         runDb $ delete $ from $ \quote -> where_
      ((quote ^. DbQuoteChannel)
      ==. val (toSqlKey $ _streamId s)
      &&. (quote ^. DbQuoteQid)
      ==. val qid
      )
...
nextQuoteId c = withStream c $ \s -> runDb $ do
...
quoteIds c = withStream c $ \s -> runDb $ fmap (fmap unValue) $ select $ from $ \quote -> do
  where_ $ (quote ^. DbQuoteChannel) ==. val (toSqlKey $ _streamId s)
