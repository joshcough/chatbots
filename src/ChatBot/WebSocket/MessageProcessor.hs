...
instance (Monad m, MonadLoggerJSON m, MonadIO m, Db m, MonadReader c m, HasConfig c, Sender m) => MessageProcessor m
  ...
  processMessage rawIrcMsg = processMessage' (cookIrcMsg rawIrcMsg)
   where
    ...
    processMessage' (Part user channel reason) = whenM (isBot user) $ do
    ...
    processMessage' (Quit user reason) = whenM (isBot user) $ do
      $(logError) "QUIT!" ["reason" .= reason, "user" .= user]
      authorize
    -- we somehow got kicked from a channel. for now, just log it.
    processMessage' (Kick user channel kicker reason) = whenM (isBot user) $ $(logError)
      "GOT KICKED!"
      ["channel" .= channel, "kicker" .= kicker, "reason" .= reason, "user" .= user]
...
processUserCommand
  ...
  -> [Text]
  -> m ()
processUserCommand chatMessage msgParams = case msgParams of
  [_, _] -> do
...
findAndRunCommand cm =
  ...
  in
    case Map.lookup name builtinCommands of
    -- builtin command
...
runBotCommand cu (BotCommand permission args body) cm rest = case permission of
...
processChatBotFrontendMessage
  :: (MonadReader c m, Sender m, MonadLoggerJSON m, MonadIO m, HasConfig c)
  => ChatBotFrontendMessage
  -> m ()
...
connectTo
  :: (MonadIO m, MonadReader c m, HasConfig c, MonadLoggerJSON m, Sender m) => ChannelName -> m ()
connectTo cn = do
...
disconnectFrom
  :: (MonadIO m, MonadReader c m, HasConfig c, MonadLoggerJSON m, Sender m) => ChannelName -> m ()
disconnectFrom cn = say cn "Goodbye!" >> send (Irc.ircPart (mkId $ getChannelNameHashed cn) "")
...
sendToWebSocket
  :: (MonadLoggerJSON m, MonadReader c m, HasConfig c, MonadIO m) => ChatMessage -> m ()
