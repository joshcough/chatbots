...
withLoggedInUser
  :: (MonadReader s m, Db.UserDb m, MonadError (AppError e) m, HasConfig s)
  => Login
  -> (User -> CookieSettings -> JWTSettings -> m b)
  -> m b
withLoggedInUser (Login e pw) f = do
  ...
  where validate hashedPw = validatePassword (encodeUtf8 hashedPw) (encodeUtf8 pw)
