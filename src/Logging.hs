...
instance Applicative m => Applicative (LoggingJSONT m) where
  ...
  {-# INLINE pure #-}
  loggerF <*> loggerA = LoggingJSONT
    $ \loggerFn -> runLoggingJSONT loggerF loggerFn <*> runLoggingJSONT loggerA loggerFn
  {-# INLINE (<*>) #-}

instance Monad m => Monad (LoggingJSONT m) where
  return = LoggingJSONT . const . return
  LoggingJSONT ma >>= f = LoggingJSONT $ \r -> do
    a <- ma r
...
instance MonadBaseControl b m => MonadBaseControl b (LoggingJSONT m) where
  ...
  liftBaseWith f = LoggingJSONT
    $ \reader' -> liftBaseWith $ \runInBase -> f $ runInBase . (\(LoggingJSONT r) -> r reader')
...
instance MonadUnliftIO m => MonadUnliftIO (LoggingJSONT m) where
  askUnliftIO =
    LoggingJSONT $ \f -> withUnliftIO $ \u -> pure (UnliftIO (unliftIO u . flip runLoggingJSONT f))
...
runStdoutLoggingJSONT minLevel sourceVersion =
  (`runLoggingJSONT` defaultOutput sourceVersion stdout)
    . filterLogger (\level _ -> level >= minLevel)
...
filterLogger :: (LogLevel -> [Pair] -> Bool) -> LoggingJSONT m a -> LoggingJSONT m a
filterLogger p (LoggingJSONT f) = LoggingJSONT
  $ \logger -> f $ \loc level msg meta -> when (p level meta) $ logger loc level msg meta
